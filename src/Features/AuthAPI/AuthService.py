...
from src.Features.AuthAPI.AccountDTO import (
    CreateAccountRequest,
    LoginAccountRequest,
    SearchAccountRequest,
    UpdateAccountRequest,
)
from src.Features.AuthAPI.AccountRepository import AccountRepository
import bcrypt
...
class AuthService:
    def __init__(
        self, repo: AccountRepository = Depends(), jwt_provider: JWTProvider = Depends()
    ):
        self.repo = repo
    ...
    async def register_account(self, dto: CreateAccountRequest):
        ...
        if existed_account:
            raise APIException(
                "Username đã tồn tại", status_code=status.HTTP_400_BAD_REQUEST
            )

        existed_email = await self.repo.find_by_email(dto.email)
        if existed_email:
            raise APIException(
                "Email đã tồn tại", status_code=status.HTTP_400_BAD_REQUEST
            )
    ...
    async def login_account(self, dto: LoginAccountRequest):
        ...
        if not account:
            raise APIException(
                "Tài khoản không tồn tại", status_code=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not is_valid:
            raise APIException(
                "Mật khẩu không hợp lệ", status_code=status.HTTP_400_BAD_REQUEST
            )
    ...
    async def edit_account(self, id: str, dto: UpdateAccountRequest):
        ...
        if not selected_account:
            raise APIException(
                "Ko tìm thấy tài khoản", status_code=status.HTTP_400_BAD_REQUEST
            )
    ...
    async def delete_account(self, id: str):
        ...
        if not user:
            raise APIException(
                "User not found", status_code=status.HTTP_400_BAD_REQUEST
            )
